...
// max & min cant be same or MCW less than MCCW
   if (config.MCW <= config.MCCW) {
     config.MCW = 1023;
     config.MCCW = 0;
   }  
// limit can't be less/more than max limit
    if (config.LCW > config.MCW ) config.LCW = config.MCW;
    if (config.LCCW < config.MCCW ) config.LCCW = config.MCCW;
//LCW cant' be less than LCCW
    if (config.LCW < config.LCCW) config.LCW = config.LCCW + 20; // this +20 gives at least little space to move
    
    digideg =  360.0 / abs(config.MCW-config.MCCW) ; 
}
//-------------------------------------------------------
void turns() {
  //read position potentiometer and calculate relative and true degrees
  dir = analogRead(POT);
  heading = (dir-config.MCCW) * digideg;
  if (heading > 360) heading = heading - 360;
  if (heading < 0) heading = 360 + heading;
  fixheading = heading + config.truefix;
  ...
  //if over limits and turning, stop
    ok = true;
    if (tcw  && (dir >= config.LCW || dir >= config.MCW)) ok = false;
    if (tccw  && (dir <= config.LCCW || dir <= config.MCCW)) ok = false;
...
*/
 ...
 if (prech == '\n') {
     if (config.negdeg == 0)writeCli("0\n999\n0.000000\n360.000000\n0.000000\n0.000000\n");
     else if (config.negdeg == 1) writeCli("0\n999\n-180.000000\n180.000000\n0.000000\n0.000000\n");
     }
  else if (prech == '+') { 
      prech = '\n';
      if (config.negdeg == 0){
...
*/  
  ...
  MYheading = (MYheading - config.truefix);
  if (MYheading > 360) MYheading = MYheading - 360;
  if (MYheading < 0) MYheading = 360 + MYheading;
  
  // from relative heading to Poti ADCreading
  int MYdir = (MYheading / digideg ) + config.MCCW;
  ShowIfDebug("FixHead:"+String(fixheading)+" RelHead:"+String(heading)+" fix:"+String(config.truefix)+" digideg:"+String(digideg)+" Poti:"+String(dir)+" MCCW:"+String(config.MCCW));
...
void set_conf(int away){
  ...
  if (RigCmd.indexOf("LCW",0) == 0){ //parameter M sets same as max , else current position
     if (RigCmd.indexOf("M",0) > 3){
       ...
       config.LCW = Mdir;
       ShowIfDebug("LCW = "+String(config.LCW));
     }
      chklimits();
      rep = "RPRT 0\n";
    }
  if (RigCmd.indexOf("LCCW",0) == 0){ //parameter M sets same as max , else current position
     if (RigCmd.indexOf("M",0) > 4){
       ...
       config.LCCW = Mdir;
       ShowIfDebug("LCCW = "+String(config.LCCW));
     }
      chklimits();
      rep = "RPRT 0\n";
    }
 ...
 if (RigCmd.indexOf("SOUTH",0) == 0){
      ...
      longTOshort(5);
      config.truefix = int(RigCmd.toInt()); 
      } else {
       config.truefix = 180 - heading; //from current position
      }
     rep = "RPRT 0\n";
    }
...
void set_reset(int away){
    ...
    //reset all parameters and  reset esp
...
void get_info(){
  // print rotatator name and all settings
     ...
     writeCli("OH1KH rot "+vers+"\nPoti :"+String(dir)+" Reldir:"+String(heading)+" Fixdir:"+String(fixheading)+
             "\nMCW:"+String(config.MCW)+"  LCW:"+String(config.LCW)+
             "\nMCCW:"+String(config.MCCW)+" LCCW:"+String(config.LCCW)+
             "\nfix:"+String(config.truefix)+"\n");
     else {
      if (prech == '+') prech = '\n';
      ...
      " MCW:"+String(config.MCW)+"  LCW:"+String(config.LCW)+
      "MCCW:"+String(config.MCCW)+" LCCW:"+String(config.LCCW)+String(prech)+
      "fix:"+String(config.truefix)+String(prech)+"RPRT 0\n");
     }    
     helpCee();
ok = true;
}  
